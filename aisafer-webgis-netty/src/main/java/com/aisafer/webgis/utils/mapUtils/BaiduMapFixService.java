...
public class BaiduMapFixService {
	...
	protected static Logger logger = Logger.getLogger(BaiduMapFixService.class);
...
*//**
	...
	public static PointLatLng reverse(double lng, double lat) {

		PointLatLng p1 = bd_decrypt(lng,lat);//转换为火星坐标
		
		p1 = MapFixService.gcjToWgs(p1.lng, p1.lat);//转换为GPS坐标
	...
	public static PointLatLng bd_encrypt(double gg_lat, double gg_lon)
	{
		...
		int bd_lat_i= (int) (bd_lat*1000000);
		int bd_lon_i= (int) (bd_lon*1000000);
        bd_lat = bd_lat_i/1000000.0;
        bd_lon = bd_lon_i/1000000.0;
	...
	public static PointLatLng fix(double lat, double lng, int corrdType) {
		...
		try {
			...
			int status = jsonArr.getIntValue("status");
			if (status == 0) {
